...
optional arguments:
  ...
  --model MODEL         BLEURT model to be used. (type: str, default: BLEURT-tiny)
  --model_storage_path MODEL_STORAGE_PATH
                        Path to the directory where models will be stored. By default
                        its saved in ~/.cache/tf/bleurt/ (default: null)
...
def t_tests_summary(
    ...
    """
    n = len(translations)
    name2id = {str(name): i for i, name in enumerate(translations)}
    grid = [[None] * n for name in translations]
    for t_test in t_test_results:
        p_value = t_test["paired_t-test"]["p_value"]
        x_id = name2id[str(t_test["x_name"])]
        y_id = name2id[str(t_test["y_name"])]
...
def score(cfg: Namespace, systems: List[Dict[str, List[str]]]) -> np.ndarray:
    ...
    """
    model = BleurtScorer(cfg.model_path if hasattr(cfg, "model_path") else None)

    if cfg.gpus > 1:
        # Create a single list that contains all systems' src, ref & translation.
        references, candidates = [], []
        for system in systems:
            references.extend(system["ref"])
            candidates.extend(system["mt"])
        ...
        seg_scores = []
        for system in systems:
            references = system["ref"]
            candidates = system["mt"]
            outputs = model.score(
                references=references,
                candidates=candidates,
                batch_size=cfg.batch_size,
            )
            seg_scores.extend(outputs)
...
def get_cfg() -> Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            + "By default its saved in ~/.cache/tf/bleurt/"
        ),
        default=None,
    )
    parser.add_argument(
